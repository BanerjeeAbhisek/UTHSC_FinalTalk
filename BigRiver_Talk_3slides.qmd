...
<div class="story-lead"><strong>BigRiverEssence is written in Julia, but people who are comfortable working in R can still use it directly.</strong> JuliaCall connects the two languages while the familiar R workflow stays in place.</div>
...
<pre><code><span class="code-fn">library</span>(JuliaCall)
julia_setup(install = <span class="code-arg">FALSE</span>)
julia_library(<span class="code-str">"BigRiverEssence"</span>)

fit &lt;- julia_call(
  <span class="code-str">"BigRiverEssence.splsda"</span>,
  X, as.character(y),
  2L, c(50L, 50L),
  scale = <span class="code-arg">TRUE</span>, levels = levels(y),
  need_return = <span class="code-str">"Julia"</span>
)

L &lt;- julia_call(<span class="code-str">"getproperty"</span>, fit,
  julia_eval(<span class="code-str">":loadings_X"</span>),
  need_return = <span class="code-str">"R"</span>)
...
<div class="bridge-caveat">Initialize once and reuse the Julia session. The first call may include Julia compilation, so compare warmed-up repeated calls.</div>
</div>
</div>

::: {.notes}
- This is not a rewrite of the R workflow. JuliaCall starts Julia inside the R session and performs automatic conversion for common objects.
- `need_return = "Julia"` keeps the large fitted object on the Julia side instead of converting it immediately.
- The second call requests only `loadings_X` as an R matrix; the example returns 50 selected genes on each component.
...
- The code on the slide was executed locally against the installed BigRiverEssence package and returned a `3116 × 2` loading matrix with 50 nonzero entries per component.
- [Sources] JuliaCall documentation: https://juliainterop.github.io/JuliaCall/ and https://juliainterop.github.io/JuliaCall/reference/call.html
:::
...
<span>Use JuliaCall to reach the same Julia implementation and bring the results back into R.</span>
</div>
</div>
...
- The audience does not need to adopt Julia wholesale. R users can call BigRiverEssence through JuliaCall and keep data preparation, reporting, and downstream work in R.
